...
export default function Home() {
  return (
    <div className="w-full bg-white min-h-screen">
      {/* Hero Section */}
      <section 
        className="relative mt-[-76px] pt-[76px] w-full h-[825px] bg-[#F0EFFC] rounded-b-[45px] flex flex-col items-center justify-center px-6 overflow-hidden"
      ...
      >
        ...
        >
          <h1 className="font-heading font-bold text-[50px] top-[196px] leading-[104%] tracking-[-3%] text-[#6761E5] mb-8">
            Rendi governabile <br /> la complessità del tuo studio.
          </h1>

          <div className="flex justify-center gap-6 mb-8">
            {[Calendar, BarChart2, MessageSquare, ClipboardList].map((Icon, i) => (
              <div key={i} className="p-5 bg-[#D6D2F7] rounded-3xl"><Icon className="text-[#8B7FE6]" size={36} /></div>
            ))}
          </div>

          <p className="font-serif font-regular text-[16px] leading-[140%] tracking-[-0.5%] text-[#545875] max-w-lg mx-auto mb-10">
          ...
          <div className="flex items-center justify-center gap-4 text-[#292E53] mb-16">
            <div className="flex -space-x-3">
              {[1, 2, 3].map((i) => (
                <div key={i} className="w-10 h-10 rounded-full bg-gray-300 border-2 border-white"></div>
              ))}
              <div className="w-10 h-10 rounded-full bg-gray-400 border-2 border-white flex items-center justify-center text-white font-bold text-sm">+</div>
            </div>
            <p className="font-serif font-medium"><strong>7.000+ professionisti</strong> gestiscono il loro studio con AlfaDocs</p>
          </div>
